  ...
  return (
    <div className="bg-slate-900 text-white min-h-screen font-sans overflow-x-hidden">
      ...
      <AnimatePresence>
        {loading && (
          ...
          >
            ...
            >
              {/* Outer Ring */}
              <div className="w-20 h-20 border-4 border-slate-300/30 border-t-cyan-500 rounded-full animate-spin"></div>
              ...
              {/* Inner Ring */}
              <div className="absolute inset-2 w-16 h-16 border-4 border-transparent border-t-purple-500 rounded-full animate-spin" 
                   style={{ animationDuration: '1.8s' }}></div>
              
              {/* Center Dot */}
              <motion.div
                className="absolute inset-0 w-20 h-20 flex items-center justify-center"
                animate={{ rotate: 360 }}
            ...
            >
              <motion.p
                className="text-2xl font-bold text-slate-200 mb-1"
            ...
            <motion.div
              className="mt-4 w-48 h-1 bg-slate-700 rounded-full overflow-hidden"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
      ...
      <section id="top" className="relative flex justify-center items-center h-screen">
        ...
        <motion.div
          className="fixed w-80 h-80 bg-gradient-to-r from-cyan-500/15 to-blue-500/15 rounded-full blur-3xl pointer-events-none z-0"
          style={{
        ...
        >
          ...
          <motion.p
            className="text-2xl mb-3 text-slate-300 drop-shadow-xl"
          ...
          <motion.div
            className="text-xl mt-4 font-light text-slate-200 drop-shadow-2xl mb-8 h-10"
            initial={{ opacity: 0, y: 15 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {[
              ...
              <motion.a
                ...
                className={`p-3 bg-slate-800/50 backdrop-blur-sm hover:bg-slate-700/70 rounded-full transition-all duration-300 hover:scale-110 hover:shadow-lg ${social.color} border border-slate-700 hover:border-cyan-500`}
                whileHover={{ 
                  scale: 1.1,
                  rotate: [0, -5, 5, 0],
                }}
          ...
          >
            <motion.a
              href="#contact"
              className="group relative px-6 py-3 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-lg font-semibold rounded-xl transition-all duration-300 uppercase tracking-wide drop-shadow-lg hover:scale-105 hover:shadow-xl hover:shadow-cyan-500/25 overflow-hidden"
            ...
            <motion.button
              ...
              className="group relative px-6 py-3 border-2 border-cyan-500 text-lg font-semibold rounded-xl transition-all duration-300 hover:bg-cyan-500 hover:text-white uppercase tracking-wide drop-shadow-lg hover:scale-105 hover:shadow-xl hover:shadow-cyan-500/25 overflow-hidden"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
      ...
      <AnimatePresence>
        {cvModalOpen && (
          ...
          >
            <motion.div 
              className="bg-slate-800 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden border border-slate-600"
              initial={{ opacity: 0, scale: 0.9, y: 30 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.9, y: 30 }}
              transition={{ duration: 0.3, ease: "easeOut" }}
            >
              <div className="flex justify-between items-center p-4 border-b border-slate-600 bg-gradient-to-r from-slate-700 to-slate-800">
                <h2 className="text-xl font-bold text-cyan-400">My Resume</h2>
                <motion.button
                  onClick={() => setCvModalOpen(false)}
                  className="text-slate-400 hover:text-red-400 text-2xl font-bold transition-colors p-1 hover:bg-slate-700 rounded-lg"
              ...
              <div className="p-4 border-t border-slate-600 flex justify-end bg-gradient-to-r from-slate-800 to-slate-700">
                <motion.a
                  href="/portfolio-nibin/nibin-resume.pdf"
                  download
                  className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-semibold rounded-lg transition-all duration-300 hover:scale-105 hover:shadow-lg flex items-center gap-2"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  <FaRocket />
